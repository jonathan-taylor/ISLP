...
def load_data(dataset):
    if dataset == 'NCI60':
        ...
        xtest = xtest.rename(columns=dict([('V%d' % d, 'X%04d' % d) for d in range(1, len(xtest.columns))]))
        ...
        xtrain = xtrain.rename(columns=dict([('V%d' % d, 'X%04d' % d) for d in range(1, len(xtest.columns))]))
