...
cdef class SequentialTreeBuilder(TreeBuilder):
    ...
    cpdef sample(self,
        ...
        Xf_all = np.empty((num_particles-1, X.shape[0])) # used to keep track of node_map of each particle tree
        samples_all = np.multiply.outer(np.ones(num_particles-1), np.arange(X.shape[0], dtype=np.intp))
        cdef SIZE_t[::1] samples
        cdef DTYPE_t[::1] Xf
        leaves_train = np.empty((X.shape[0], num_particles-1), dtype=np.intp)
        ...
        for _ in range(num_particles-1):
            particles.append((Tree(num_features, classes, 1), LifoQueue(maxsize=2047)))
        particles = np.array(particles)
        cdef DTYPE_t[::1] weights = np.empty(num_particles, dtype=X.dtype)
        ...
        for particle_idx in particle_indices:
            tree, queue = particles[particle_idx-1]
        ...
        while any_expansion_nodes:
            ...
            for particle_idx in particle_indices:
                ...
                tree, queue = particles[particle_idx]
                stack_record = queue.get()
                ...
                if ((start < end) and 
                    ...
                    # get current sorted features and samples
                    Xf = Xf_all[particle_idx-1]
                    samples = samples_all[particle_idx-1]
                    ...
                    for idx in range(start, end):
                        leaves_train[samples[idx], particle_idx-1] = node_id
            ...
            any_expansion_nodes = np.any([queue.qsize() > 0 for _, queue in particles])

        if rc == -1:
            raise MemoryError()

        return cur_tree # , loglikelihood, np.asarray(leaves_train)
...
cdef set_values_tree(Tree tree,
    ...
    print(np.array(leaf_map), 'count', tree.node_count)
    ...
    for leaf_id in range(node_count):
        if tree.nodes[leaf_id].left_child == _TREE_LEAF: # we are in a leaf
            ...
            tree.value[leaf_id] = response_sum[leaf_id] / n_node_samples
